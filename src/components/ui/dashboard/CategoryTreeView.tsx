			...
			<>
				...
				<div className="group flex items-center space-x-2 p-2 rounded-md hover:bg-muted transition-colors border border-transparent hover:border-border">
					...
					<div className="flex flex-col flex-1 min-w-0">
						...
						<span className="text-xs text-muted-foreground truncate">{node.slug}</span>
					</div>
				</div>
						...
						{node.children.map((child) => (
							...
							>
								...
								<div className="flex flex-col flex-1 min-w-0">
									...
									<span className="text-xs text-muted-foreground truncate">{child.slug}</span>
								</div>
							</div>
						))}
					</div>
	...
	return (
		<div className="border border-border rounded-lg p-4 bg-transparent">
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
				{tree.map((node) => (
					<CategoryGroup key={node.id} node={node} onEdit={onEdit} onDelete={onDelete} />
				))}
			</div>
		</div>
	);
