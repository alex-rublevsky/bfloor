...
>(({ className, ...props }, ref) => (
	<DrawerPrimitive.Overlay
		ref={ref}
		className={cn(
			"fixed inset-0 z-[100] bg-background/45 backdrop-blur-xs transition-all duration-300",
			className,
		)}
		{...props}
	/>
...
>(({ className, children, width, ...props }, ref) => (
	<DrawerPortal>
		...
		<DrawerPrimitive.Content
			ref={ref}
			className={cn(
				"fixed inset-x-0 bottom-0 z-[101] mt-24 flex flex-col rounded-t-[10px] border border-border bg-background",
...
const DrawerBody = ({
	...
	<div
		className={cn("flex-1 overflow-y-auto px-4 pt-2 pb-6 min-h-0", className)}
		{...props}
	/>
);
