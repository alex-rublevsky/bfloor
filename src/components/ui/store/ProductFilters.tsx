...
const ProductFilters = memo(function ProductFilters({
	...
	return (
		<>
			...
			>
				{hasAnyActiveFilters && (
					<Button variant="accent" size="sm" ref={resetRef} onClick={resetAll}>
						Сбросить
					</Button>
				)}
			</div>
			...
			<div className="hidden md:block fixed left-0 right-0 bottom-0 z-[35] group pointer-events-none">
				...
				<section
					className="pointer-events-auto fixed left-0 right-0 bottom-0 w-screen transform-gpu translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-[cubic-bezier(0.215,0.61,0.355,1)] z-[36]"
					aria-label="Фильтры продуктов"
					tabIndex={-1}
					onMouseEnter={handleMouseEnter}
					onMouseLeave={handleMouseLeave}
				...
				>
					<div
						className="w-full h-full border-t border-border bg-background/87 backdrop-blur-sm shadow-2xl"
						style={{ margin: 0 }}
					>
						<div className="mx-auto w-full max-w-[1600px] px-4 py-4">
							<div className="space-y-6">
								{/* Sort and Price in a row at the top */}
								<div className="grid grid-cols-2 gap-6">
									...
									<div>
										<Slider
											...
											label="Диапазон цен"
											valueDisplay={
												<output className="text-sm font-medium tabular-nums">{`${currentPriceRange[0]} р - ${currentPriceRange[1]} р`}</output>
											}
										/>
									</div>
								</div>
								...
								{/* Brands and Collections in a row */}
								<div className="grid grid-cols-2 gap-6">
