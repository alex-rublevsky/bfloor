...
function RouteComponent() {
  ...
  const { crud, createForm, editForm } = useDashboardForm<StoreLocationFormData>(
    {
      address: "",
      description: "",
      openingHours: "",
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      closeCreateDrawer();
      queryClient.invalidateQueries({ queryKey: ["bfloorDashboardStoreLocations"] });
    } catch (err) {
  ...
  const handleUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    crud.startSubmitting();
    try {
      await updateStoreLocation({ data: { id: editingId, data: editForm.formData } });
      ...
      <div className="px-4">
        <div className="flex items-center gap-3 text-sm text-muted-foreground">
          <div className="flex items-center gap-1">
            <span>
              {locations ? `${locations.length} адресов` : '0 адресов'}
            </span>
          </div>
        </div>
      </div>
        ...
        {locations.map((loc) => (
          <div key={loc.id} className="border rounded-lg p-4 bg-card space-y-3">
            <div className="space-y-1">
              ...
              {loc.description && (
                <div className="text-sm text-muted-foreground break-words">{loc.description}</div>
              )}
              {loc.openingHours && (
                <div className="text-sm text-muted-foreground break-words">{loc.openingHours}</div>
              )}
              {!loc.isActive && (
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1" onClick={() => handleEdit(loc)}>
                Редактировать
              </Button>
              <Button variant="destructive" size="sm" className="flex-1" onClick={() => handleDeleteClick(loc)}>
                Удалить
              </Button>
            </div>
          </div>
