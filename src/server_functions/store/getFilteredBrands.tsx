 ...
 */
export const getFilteredBrands = createServerFn({ method: "GET" })
	.inputValidator(
		(
			data: {
				categorySlug?: string;
	...
	.handler(async ({ data = {} }) => {
		try {
			...
			const conditions: SQL[] = [
				sql`${products.brandSlug} IS NOT NULL`,
			...
			// Uses INNER JOIN with brands and optional LEFT JOIN with product_store_locations
			let query = db
				.selectDistinct({
					id: brands.id,
					name: brands.name,
