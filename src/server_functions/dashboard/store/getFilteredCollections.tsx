...
/**
 * Get collections filtered by current filter selection (Dashboard version - includes inactive products)
 * Only returns collections that have products matching the current filters
 * Uses a single JOIN query for maximum performance
 */
export const getFilteredCollectionsDashboard = createServerFn({ method: "GET" })
	.inputValidator(
		(
			data: {
				categorySlug?: string;
	...
	.handler(async ({ data = {} }) => {
		try {
			const db = DB();
			const conditions: SQL[] = [sql`${products.collectionSlug} IS NOT NULL`];
			...
			console.error("Error fetching filtered collections (dashboard):", error);
